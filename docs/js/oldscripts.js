...
const elemList = ['fire', 'water', 'earth', 'air', 'mind', 'body'];
const weakDict = new Object(); weakDict['fire'] = 'water, air';weakDict['water'] = 'body, earth';weakDict['earth'] = 'fire, mind'; weakDict['air'] = 'body'; weakDict['mind'] = ''; weakDict['body'] = 'fire';
...
class Monster{
	constructor(scale, elem, id, shape = shapeList[random(0, shapeList.length-1)]){
		...
		this.stats = new Stats(scale, this.type.shape);
		this.moves = new Object();
		
		this.moves['basic'] = '3/90';
		this.moves['heavy'] = '7/75';
	}
	
	//comabt functions
	attack(move, target){
		if(target.stats.hp <= 0){write('<br>enemy killed <br> Click to continue', true); return};
		let txt = move.innerText.split('/'); let amt = parseInt(txt[0]);

		if(random(0,100)<txt[1]){ write('<br>'+this.id+ ' attacked', true); target.damage(amt*this.stats.atk, this.type.elem)}
	...
	kill(){
		...
		if(this.id == 'enemy'){this.rewards(); user.enemy = false};
		if(this.id == 'boss'){win()};
	...
	expUp(){
		this.stats.exp++;
		if(this.stats.exp >= 5*this.stats.level){this.levelUp(); this.stats.exp = 0};
	}
...
class Encounter{
	constructor(){
		...
		this.elem = elemList[random(0, elemList.length-1)];
	}
	init(){
...
function showStats(){
	...
	for(let monster in user.party){
		console.log(user.party[monster]);
