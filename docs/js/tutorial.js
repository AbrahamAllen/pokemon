...
class Monster{
	...
	levelUp(){
		...
		if(this.stats.lvl == 4){this.moves.push(moveDict[this.type.shape][2])};
		if(this.stats.lvl == 6){this.moves.push(moveDict[this.type.elem][0])};
	...
	col(){
		for(let encounter of Object.values(mapCol)){
			if(this.x > encounter[0] && this.x < encounter[0]+encounter[2] && this.y > encounter[1] && this.y < encounter[1]+encounter[3]){
				...
				if(encounter[4].type == 'portal'){alert('Good Luck!'); window.location.assign('game.html') ; return};
...
class Encounter{
	...
	init(){
		...
		document.getElementById('tutorialmap').appendChild(div);
	...
	makeParent(){
		...
		document.getElementById('tutorialmap').appendChild(par);
	}
...
user.x = 200;
user.y = 20;
user.party[0] = new Monster(10, 'water', 'player');
...
let fire = new Encounter('electric');
fire.x = 200;
fire.y = 600;
fire.w = 200;
fire.h = 200;
...
let startGame = new Interactable('portal');
startGame.x = 200;
startGame.y = 900;
startGame.init();